#!/bin/bash
#
#SBATCH --job-name=BRB_MultiQC_FullRun
#SBATCH --output=BRB_MultiQC_FullRun_%A_%a.out  # %a inserts the job number
#SBATCH --error=BRB_MultiQC_FullRun_%A_%a.err
#SBATCH --array=1%1  
...
then
    echo ""
    echo "ERROR: Too few arguments passed to BRB_MultiQC_FullRun_0804.sh."
    echo "    You provided $#, 2  required."
    echo ""
    echo "@Usage: sbatch BRB_MultiQC_FullRun_240116.sh PROJECT_DIR LIBRARY_NAME"
...
echo 'Fastqc Version: ' ${fastqcVersion}
echo ""
...
config_fullrun_src="${config_dir}/multiqc_config_fullrun_240116.yaml"

# set destination config file location
config_standard_tgt="${PROJECT_DIR}/MultiQC/multiqc_config_fullrun.yaml"
...
LIBRARY_NAME=${LIBRARY_NAME} envsubst  < ${config_fullrun_src} > ${config_standard_tgt}
